  ...
  return (
    <div className="space-y-5">
      ...
      <Card title="Strong news sentiment" subtitle="FinBERT-scored headlines, |score| ≥ 0.5, last 30 days">
        {d.sentiment_movers.length ? (
          <ul className="divide-y divide-edge/60">
            {d.sentiment_movers.map((m: any, i: number) => (
              <li key={i} className="py-2 flex gap-3 items-start">
                <Link to={`/stock/${m.stock_id}`} className="font-medium text-indigo-300 hover:text-indigo-200 w-28 shrink-0">{m.symbol}</Link>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card title="Momentum leaders" subtitle="Top composite score (monthly model)">
          {d.momentum.length ? (
            <table className="w-full text-sm">
              <thead><tr><th className="th">Symbol</th><th className="th text-right">Score</th><th className="th text-right">RSI rank</th><th className="th text-right">MACD rank</th></tr></thead>
              <tbody>
                {d.momentum.map((m: any, i: number) => (
                  <tr key={i}>
        ...
        <Card title="Notable bulk & block deals" subtitle="Last 30 days, outside the watchlist">
          {d.recent_deals.length ? (
            <table className="w-full text-sm">
              <thead><tr><th className="th">Symbol</th><th className="th">Txn</th><th className="th text-right">Qty</th><th className="th">Date</th></tr></thead>
              <tbody>
                {d.recent_deals.map((b: any, i: number) => (
                  <tr key={i}>
